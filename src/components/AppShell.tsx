...
import { LayoutDashboard, Bot, Users, Send, LogOut, Search, Bell, Menu } from "lucide-react";
...
export function AppShell() {
    ...
    <div className="flex flex-col h-full bg-sidebar">
      ...
      <div className="px-5 h-14 flex items-center gap-2.5 border-b border-sidebar-border">
        ...
        <div className="leading-tight">
          <div className="text-[13px] font-semibold tracking-tight">VisitaIA</div>
          <div className="text-[10px] text-muted-foreground font-mono uppercase tracking-[0.15em]">Plataforma</div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen flex bg-background text-foreground">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex w-[248px] shrink-0 border-r border-sidebar-border bg-sidebar flex-col">
        <SidebarContent />
      </aside>

      <main className="flex-1 flex flex-col min-w-0">
        {/* Top bar */}
        <header className="h-14 border-b border-border flex items-center px-4 md:px-6 gap-4 sticky top-0 z-10 bg-background/80 backdrop-blur-md">
          ...
          <div className="flex items-center gap-2 text-[13px] overflow-hidden">
            ...
            <span className="font-medium truncate">{current?.label ?? "Painel"}</span>
          </div>
